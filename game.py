...
class Game:
    ...
    def draw(self):
        ...
        # draw the score
        ui.draw_text(self.surface, f"Punkte : {self.score}", (5, 5), COLORS["score"], font=FONTS["medium"],
                    shadow=True, shadow_color=(255,255,255))
        # draw the time left
        timer_text_color = (160, 40, 0) if self.time_left < 5 else COLORS["timer"] # change the text color if less than 5 s left
        ui.draw_text(self.surface, f"Übrige Zeit : {self.time_left}", (SCREEN_WIDTH//2, 5),  timer_text_color, font=FONTS["medium"],
                    shadow=True, shadow_color=(255,255,255))
    ...
    def update(self):
        ...
        if self.time_left > 0:
            ...
            print("Hand geschlossen", self.hand.left_click)
            ...
            if ui.button(self.surface, 540, "Weiter spielen", click_sound=self.sounds["slap"]):
                return "menu"
